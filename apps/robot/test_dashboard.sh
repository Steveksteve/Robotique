...
check_services() {
    ...
    if [ -z "$ros_core_status" ] || [ -z "$rosbridge_status" ] || [ -z "$dashboard_status" ]; then
        echo -e "${RED}[ERREUR] Services not running. Start with: docker-compose up -d${NC}"
        exit 1
    fi
    
    echo -e "${GREEN}[OK] All services running${NC}"
}
...
test_dashboard_http() {
    ...
    if curl -s http://localhost:8080 > /dev/null; then
        echo -e "${GREEN}[OK] Dashboard HTTP 200 OK${NC}"
        return 0
    else
        echo -e "${RED}[ERREUR] Dashboard HTTP failed${NC}"
        return 1
    fi
}
...
try:
    ...
    ws.close()
    print("[OK] RosBridge WebSocket 200 OK")
    sys.exit(0)
except Exception as e:
    print(f"[ERREUR] RosBridge WebSocket failed: {e}")
    sys.exit(1)
PYEOF
}
...
test_browser_open() {
    ...
    echo -e "${GREEN}[OK] Open http://localhost:8080 manually if browser didn't open${NC}"
}
...
test_service_health() {
    ...
    echo -e "${GREEN}[OK] Services should show 'healthy' in STATUS${NC}"
}
...
if [ ! -f "docker-compose.yml" ]; then
    echo -e "${RED}[ERREUR] docker-compose.yml not found${NC}"
